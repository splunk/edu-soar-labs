...
def on_start(container):
    ...
    # call 'playbook_log_prior_hashes__advanced_solution_lab_1__1' block
    playbook_log_prior_hashes__advanced_solution_lab_1__1(container=container)
...
def source_country_filter(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    if matched_artifacts_2 or matched_results_2:
        update_finding_or_investigation_1(action=action, success=success, container=container, results=results, handle=handle, filtered_artifacts=matched_artifacts_2, filtered_results=matched_results_2)
        ...
        if finding_data_item[0] is not None:
            parameters.append({
                ...
                "incident_note": {
                    ...
                    "content": "Malicious content found from low risk source",
                },
            })
...
@phantom.playbook_block()
def playbook_log_prior_hashes__advanced_solution_lab_1__1(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    phantom.debug("playbook_log_prior_hashes__advanced_solution_lab_1__1() called")

    finding_data = phantom.collect2(container=container, datapath=["finding:consolidated_findings.fileHash"])

    finding_consolidated_findings_filehash = [item[0] for item in finding_data]

    inputs = {
        "hash": finding_consolidated_findings_filehash,
    }
    ...
    # call playbook "local/Log Prior Hashes (Advanced Solution Lab 1)", returns the playbook_run_id
    playbook_run_id = phantom.playbook("local/Log Prior Hashes (Advanced Solution Lab 1)", container=container, inputs=inputs)
